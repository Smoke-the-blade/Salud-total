...
function DoctorDashboard() {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:3001/api/doctores/turnos/${doctor.id}`)
      .then((res) => {
    ...
    axios.get(`http://localhost:3001/api/doctores/horarios/${doctor.id}`)
  ...
  const cambiarEstado = async (turnoId, nuevoEstado) => {
    ...
    try {
      await axios.put(`http://localhost:3001/api/doctores/turnos/${turnoId}`, { estado: nuevoEstado });
      setMensaje('Estado actualizado.');

      const res = await axios.get(`http://localhost:3001/api/doctores/turnos/${doctor.id}`);
      setTodosLosTurnos(res.data);
  ...
  const cambiarContrasena = async () => {
    ...
    try {
      await axios.put(`http://localhost:3001/api/doctores/cambiar-contrasena/${doctor.id}`, {
  ...
  const eliminarHorario = (index) => {
    ...
    axios.put(`http://localhost:3001/api/doctores/actualizar-horarios/${doctor.id}`, { horarios: nuevos })
  ...
  const guardarHorarios = async () => {
    try {
      ...
      await axios.put(`http://localhost:3001/api/doctores/actualizar-horarios/${doctor.id}`, { horarios: nuevos });
      setHorarios(nuevos);
  ...
  const enviarFormulario = async () => {
    ...
    try {
      const res = await axios.post('http://localhost:3001/api/doctores/formulario', {
        doctor_id: doctor.id,
  ...
  const buscarFormularios = async () => {
    ...
    try {
      const res = await axios.get(`http://localhost:3001/api/doctores/formularios-nombre/${encodeURIComponent(nombrePaciente.trim())}`);
  ...
  const guardarEdicionFormulario = async () => {
    try {
      await axios.put(`http://localhost:3001/api/doctores/formulario/${editarFormularioId}`, {
        contenido: formularioEditado
      });
