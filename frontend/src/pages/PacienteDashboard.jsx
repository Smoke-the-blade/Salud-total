...
function PacienteDashboard() {
  ...
  useEffect(() => {
    if (paciente?.id) {
      axios.get(`http://localhost:3001/api/pacientes/turnos/${paciente.id}`)
  ...
  const handleEspecialidad = (e) => {
    ...
    setDoctores([]);
    setMedico('');
    axios.get(`http://localhost:3001/api/admin/medicos-por-especialidad/${id}`)
  ...
  const handleMedico = (e) => {
    const id = e.target.value;
    setMedico(id);
    axios.get(`http://localhost:3001/api/doctores/horarios/${id}`)
      .then(res => setHorariosMedico(res.data));
  };

  useEffect(() => {
    if (fecha && medico) {
      axios.get(`http://localhost:3001/api/doctores/ocupados/${medico}/${fecha}`)
  ...
  const solicitarTurno = async () => {
    ...
    try {
      await axios.post('http://localhost:3001/api/pacientes/solicitar-turno', {
      ...
      const ocupadas = await axios.get(`http://localhost:3001/api/doctores/ocupados/${medico}/${fecha}`);
      setHorasOcupadas(ocupadas.data);

      const turnosActualizados = await axios.get(`http://localhost:3001/api/pacientes/turnos/${paciente.id}`);
  ...
  const cancelarTurno = async (id) => {
    try {
      await axios.patch(`http://localhost:3001/api/pacientes/turno/cancelar/${id}`);
  ...
  const guardarCambiosTurno = async (id) => {
    ...
    try {
      await axios.put(`http://localhost:3001/api/pacientes/turno/${id}`, {
        nuevaFecha,
        nuevaHora
      });
  ...
  const actualizarPerfil = async (tipo) => {
    ...
    try {
      await axios.put(`http://localhost:3001/api/pacientes/actualizar/${paciente.id}`, payload);
